...
VALUES
    (7001, 5001, '고혈압약A', 1, '정제', '아침 복용', NULL, NULL, NULL),
...
SELECT
    s.schedule_id,
    TIMESTAMP(DATE_ADD(DATE('2025-11-01'), INTERVAL d.day_offset DAY), s.slot_time) AS scheduled_dt,
    CASE
    WHEN d.day_no <= 10 THEN
    DATE_ADD(TIMESTAMP(DATE_ADD(DATE('2025-11-01'), INTERVAL d.day_offset DAY), s.slot_time), INTERVAL 5 MINUTE)
    WHEN d.day_no BETWEEN 11 AND 20 AND s.schedule_id IN (9001, 9003) THEN
    DATE_ADD(TIMESTAMP(DATE_ADD(DATE('2025-11-01'), INTERVAL d.day_offset DAY), s.slot_time), INTERVAL 15 MINUTE)
    ELSE NULL
END AS taken_dt,
    CASE
...
FROM (
         SELECT 0 AS day_offset, 1  AS day_no UNION ALL
         SELECT 1, 2  UNION ALL SELECT 2, 3  UNION ALL SELECT 3, 4  UNION ALL SELECT 4, 5
         ...
         UNION ALL SELECT 21, 22 UNION ALL SELECT 22, 23 UNION ALL SELECT 23, 24 UNION ALL SELECT 24, 25
         UNION ALL SELECT 25, 26 UNION ALL SELECT 26, 27 UNION ALL SELECT 27, 28 UNION ALL SELECT 28, 29
         UNION ALL SELECT 29, 30
     ) d
         CROSS JOIN (
